...
import { AfterViewInit, ChangeDetectionStrategy, ChangeDetectorRef, Component, ElementRef, HostBinding, HostListener, OnChanges, OnDestroy, OnInit, QueryList, Renderer2, SimpleChanges, ViewChildren, ViewEncapsulation, DOCUMENT, inject, input, output, viewChild, model } from '@angular/core';
...
export class FuseVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    readonly inner = model<boolean>(false);
    ...
    readonly opened = model<boolean>(true);
    readonly position = input<FuseVerticalNavigationPosition>('left');
    readonly transparentOverlay = model<boolean>(false);
    ...
    ngOnChanges(changes: SimpleChanges): void
    {
        ...
        if ( 'inner' in changes )
        {
            ...
            this.inner.set(coerceBooleanProperty(changes.inner.currentValue));
        }
        ...
        {
            // Coerce the value to a boolean
            this.opened.set(coerceBooleanProperty(changes.opened.currentValue));
        ...
        {
            // Coerce the value to a boolean
            this.transparentOverlay.set(coerceBooleanProperty(changes.transparentOverlay.currentValue));
        }
    }
